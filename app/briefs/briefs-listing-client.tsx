...
export default function BriefsListingClient({
  ...
  canFavorite: boolean;
  appliedStatusByBriefId: Record<string, string>;
}) {
      ...
      <div className="flex flex-col lg:flex-row gap-8 lg:gap-10">
        <main className="flex-1 min-w-0 lg:w-2/3 space-y-4 order-2 lg:order-1">
          {filtered.length === 0 ? (
            ...
            filtered.map((b) => {
              const appliedStatus = appliedStatusByBriefId[b.id];
              const appliedLabel = appliedStatus
                ? `${appliedStatus[0]?.toUpperCase()}${appliedStatus.slice(1)}`
                : null;
              return (
                <article
                  key={b.id}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {appliedStatus ? (
                    <span
                      className="inline-flex items-center justify-center rounded-lg border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-medium text-indigo-700"
                      title="You already applied to this brief."
                    >
                      Already applied{appliedLabel ? ` · ${appliedLabel}` : ""}
                    </span>
                  ) : (
                    <Link
